...
on_error() { printf 'Установка остановлена на строке %s. Данные не удалялись.\n' "$1" >&2; }
trap 'on_error "$LINENO"' ERR

if [[ "${EUID}" -ne 0 ]]; then fail "запустите команду через sudo: sudo ./install.sh"; fi
if [[ ! -f "${COMPOSE_FILE}" ]]; then fail "docker-compose.prod.yml не найден"; fi
RESUME=false
if [[ -e "${ENV_FILE}" ]]; then
  if [[ -e "${INSTALL_MARKER}" ]]; then fail "система уже установлена. Для обновления используйте sudo ./scripts/update.sh"; fi
  RESUME=true
fi
...
install_docker() {
  ...
  if ! docker compose version >/dev/null 2>&1; then apt-get install -y docker-compose-v2 || apt-get install -y docker-compose-plugin; fi
...
for _attempt in $(seq 1 60); do
  if curl -fsS --max-time 5 "https://${APP_DOMAIN}/api/v1/health/" >/dev/null 2>&1; then READY=true; break; fi
  sleep 2
done
printf 'Логин администратора: %s\n' "${ADMIN_USERNAME}"
if [[ "${GENERATED_ADMIN_PASSWORD}" == true ]]; then printf 'Сгенерированный пароль: %s\nСохраните его сейчас: повторно он не выводится.\n' "${ADMIN_PASSWORD}"; fi
if [[ "${READY}" == true ]]; then
  touch "${INSTALL_MARKER}"; chmod 600 "${INSTALL_MARKER}"
  printf '\nУстановка завершена.\nСайт: https://%s\nWorkspace: https://%s/app\nAdmin Console: https://%s/admin-console\nDjango admin: https://%s/admin/\nMFA администратора: https://%s/security/mfa\n' "${APP_DOMAIN}" "${APP_DOMAIN}" "${APP_DOMAIN}" "${APP_DOMAIN}" "${APP_DOMAIN}"
  ...
  printf 'Проверьте DNS и журнал: sudo docker compose --env-file .env.production -f docker-compose.prod.yml logs caddy\nПосле исправления повторите: sudo ./install.sh\n' >&2
  exit 1
fi
printf 'До коммерческого запуска заполните SMTP, реквизиты продавца, AI-провайдеров/цены, YooKassa, включите MFA и закройте compliance signoffs/drills.\n'
