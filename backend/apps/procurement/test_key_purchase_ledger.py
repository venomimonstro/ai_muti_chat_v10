...
class KeyPurchaseLedgerTests(TestCase):
    def test_key_linked_account_drives_runtime_key_and_fifo_purchase_cost(self):
        ...
        self.assertEqual(provider.get_api_key(), "paid-secret")
        ...
        self.assertTrue(first.document_number.startswith("API-"))
        self.assertEqual(first.total_cash_outlay_rub, Decimal("1000.0000"))
        self.assertEqual(second.total_cash_outlay_rub, Decimal("2000.0000"))
        ...
        allocations = list(
            ProviderSpendAllocation.objects.filter(spend=spend).order_by("purchase__purchased_at", "created_at")
        )
        self.assertEqual(len(allocations), 2)
        self.assertEqual(allocations[0].purchase_id, first.id)
        self.assertEqual(allocations[0].native_amount, Decimal("10.000000"))
        self.assertEqual(allocations[0].economic_cost_rub, Decimal("1000.0000"))
        self.assertEqual(allocations[1].purchase_id, second.id)
        self.assertEqual(allocations[1].native_amount, Decimal("2.000000"))
        self.assertEqual(allocations[1].economic_cost_rub, Decimal("400.0000"))
        self.assertEqual(spend.economic_cost_rub, Decimal("1400.0000"))
        self.assertEqual(spend.customer_charge_rub, Decimal("2400.0000"))
