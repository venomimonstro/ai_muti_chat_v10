...
class EchoProviderAdapter:
    ...
    def health_check(self): return AdapterHealth(healthy=True,latency_ms=0)
    def capabilities(self): return {"text","streaming"}
...
def _http_error(exc: httpx.HTTPError) -> ProviderError:
    ...
    if status==429:return ProviderError("Provider rate limit",code="rate_limited",retryable=True)
    if status is not None and 400<=status<500:return ProviderError("Provider rejected request",code=f"http_{status}",retryable=False)
    if isinstance(exc,httpx.TimeoutException):return ProviderError("Provider timeout",code="timeout",retryable=True)
...
class OpenAIResponsesAdapter(HTTPAdapter):
        ...
        try:
            with httpx.stream("POST",f"{self.base_url}/responses",headers=headers,json=payload,timeout=settings.AI_PROVIDER_TIMEOUT_SECONDS) as response:
                ...
                for line in response.iter_lines():
                    ...
                    event=json.loads(data);event_type=event.get("type")
                    if event_type=="response.output_text.delta":yield ProviderStreamEvent(kind="delta",text_delta=event.get("delta",""))
    ...
    def health_check(self):return self._health_get(url=f"{self.base_url}/models",headers={"Authorization":f"Bearer {self.api_key}"})
...
class AnthropicMessagesAdapter(HTTPAdapter):
    def __init__(self,*,api_key:str,base_url:str="https://api.anthropic.com/v1"):
        ...
        self.api_key=api_key;self.base_url=base_url.rstrip("/")
    @property
    def headers(self):return {"x-api-key":self.api_key,"anthropic-version":"2023-06-01","content-type":"application/json"}
    def stream(self,*,model:str,messages:list[dict],max_output_tokens:int):
        ...
        try:
            with httpx.stream("POST",f"{self.base_url}/messages",headers=self.headers,json=payload,timeout=settings.AI_PROVIDER_TIMEOUT_SECONDS) as response:
                ...
                for line in response.iter_lines():
                    if not line.startswith("data:"):continue
                    event=json.loads(line[5:].strip());event_type=event.get("type")
                    if event_type=="message_start":
                        envelope=event.get("message") or {};request_id=envelope.get("id","");input_tokens=(envelope.get("usage") or {}).get("input_tokens",0)
                    elif event_type=="content_block_delta":
                        delta=event.get("delta") or {}
                        if delta.get("type")=="text_delta":yield ProviderStreamEvent(kind="delta",text_delta=delta.get("text",""))
                    elif event_type=="message_delta":output_tokens=(event.get("usage") or {}).get("output_tokens",output_tokens)
                    elif event_type=="error":
                        error=event.get("error") or {};raise ProviderError("Provider stream failed",code=error.get("type","provider_error"),retryable=error.get("type")!="invalid_request_error")
                    elif event_type=="message_stop":yield ProviderStreamEvent(kind="completed",provider_request_id=request_id,input_tokens=input_tokens,output_tokens=output_tokens)
        except httpx.HTTPError as exc:raise _http_error(exc) from exc
        except json.JSONDecodeError as exc:raise ProviderError("Invalid provider stream",code="invalid_stream") from exc
    def generate(self,*,model:str,messages:list[dict],max_output_tokens:int):return _collect(self,model=model,messages=messages,max_output_tokens=max_output_tokens)
    def health_check(self):return self._health_get(url=f"{self.base_url}/models",headers=self.headers)
    def capabilities(self):return {"text","streaming","vision","tools"}
...
class DeepSeekChatAdapter(HTTPAdapter):
    ...
    def stream(self,*,model:str,messages:list[dict],max_output_tokens:int):
        if any(any(block.get("type")=="image" for block in _generic_blocks(item.get("content"))) for item in messages):raise ProviderError("Model does not support image input",code="vision_unsupported",retryable=False)
        ...
        try:
            with httpx.stream("POST",f"{self.base_url}/chat/completions",headers=self.headers,json=payload,timeout=settings.AI_PROVIDER_TIMEOUT_SECONDS) as response:
                ...
                for line in response.iter_lines():
                    ...
                    data=line[5:].strip()
                    if not data or data=="[DONE]":continue
                    event=json.loads(data);request_id=event.get("id",request_id);usage=event.get("usage") or usage;choices=event.get("choices") or []
                    if choices:
...
class XAIChatAdapter(DeepSeekChatAdapter):
    def __init__(self,*,api_key:str,base_url:str="https://api.x.ai/v1"):super().__init__(api_key=api_key,base_url=base_url)
    def stream(self,*,model:str,messages:list[dict],max_output_tokens:int):
...
class GeminiGenerateContentAdapter(HTTPAdapter):
    def __init__(self,*,api_key:str,base_url:str="https://generativelanguage.googleapis.com/v1beta"):
        ...
        self.api_key=api_key;self.base_url=base_url.rstrip("/")
    @property
    def headers(self):return {"x-goog-api-key":self.api_key,"Content-Type":"application/json"}
    def stream(self,*,model:str,messages:list[dict],max_output_tokens:int):
        system="\n\n".join(_text_only(item["content"]) for item in messages if item["role"]=="system")
        ...
        try:
            with httpx.stream("POST",f"{self.base_url}/models/{model}:streamGenerateContent?alt=sse",headers=self.headers,json=payload,timeout=settings.AI_PROVIDER_TIMEOUT_SECONDS) as response:
                ...
                for line in response.iter_lines():
                    ...
                    event=json.loads(data);request_id=event.get("responseId",request_id);usage=event.get("usageMetadata") or usage
                    blocked=(event.get("promptFeedback") or {}).get("blockReason")
                    if blocked:raise ProviderError("Provider blocked prompt",code=f"blocked_{blocked.lower()}",retryable=False)
                    candidates=event.get("candidates") or []
                    if not candidates:continue
                    parts=((candidates[0].get("content") or {}).get("parts") or [])
                    for part in parts:
                        if not part.get("thought") and part.get("text"):yield ProviderStreamEvent(kind="delta",text_delta=part["text"])
...
def _collect(adapter,*,model:str,messages:list[dict],max_output_tokens:int):
    ...
    for event in adapter.stream(model=model,messages=messages,max_output_tokens=max_output_tokens):
        if event.kind=="delta":text+=event.text_delta
        else:completed=event
    if completed is None:raise ProviderError("Provider stream ended without completion event",code="invalid_stream")
...
def adapter_for(model: AIModel):
    ...
    if provider.adapter_type==Provider.AdapterType.GEMINI_GENERATE_CONTENT:return GeminiGenerateContentAdapter(api_key=api_key,base_url=provider.api_base_url or os.getenv("GEMINI_API_BASE_URL","https://generativelanguage.googleapis.com/v1beta"))
    if provider.adapter_type==Provider.AdapterType.XAI_CHAT:return XAIChatAdapter(api_key=api_key,base_url=provider.api_base_url or os.getenv("XAI_API_BASE_URL","https://api.x.ai/v1"))
