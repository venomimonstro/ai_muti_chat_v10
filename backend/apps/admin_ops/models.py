...
class OperationalDrillEvidence(models.Model):
    class Kind(models.TextChoices):
        LOAD = "load", "Load smoke"
        CHAOS = "chaos", "Chaos restart"
        COMMERCIAL_E2E = "commercial_e2e", "Commercial E2E"
        PROVIDER_OUTAGE = "provider_outage", "Provider outage"
        DUPLICATE_WEBHOOK = "duplicate_webhook", "Duplicate payment webhook"
        PAYMENT_FAILURE = "payment_failure", "Payment failure"
        REFUND = "refund", "Refund flow"
        STALE_RECOVERY = "stale_recovery", "Stale operation recovery"
    ...
    def save(self, *args, **kwargs):
        if self.pk and type(self).objects.filter(pk=self.pk).exists():
            raise ValidationError("Operational drill evidence is immutable")
        return super().save(*args, **kwargs)

    def delete(self, *args, **kwargs):
        raise ValidationError("Operational drill evidence cannot be deleted")
