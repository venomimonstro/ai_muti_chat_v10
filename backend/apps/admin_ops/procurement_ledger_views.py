...
def _purchase_payload(purchase):
    ...
    remaining = max(ZERO, purchase.credit_native - consumed)
...
class ProcurementLedgerView(AdminAPIView):
    def get(self, request):
        ...
        purchases = list(
            ProviderPurchase.objects.select_related("account__provider", "account__api_key")
            .order_by("-purchased_at", "-created_at")[:500]
        )
        purchase_rows = [_purchase_payload(item) for item in purchases]
        cash = sum((item.total_cash_outlay_rub for item in purchases), ZERO)
        fees = sum((item.fees_rub for item in purchases), ZERO)
        credit = sum((item.credit_native for item in purchases), ZERO)
        consumed = sum((Decimal(item["consumed_native"]) for item in purchase_rows), ZERO)
        revenue = sum((Decimal(item["realized_revenue_rub"]) for item in purchase_rows), ZERO)
        cost = sum((Decimal(item["realized_cost_rub"]) for item in purchase_rows), ZERO)
        profit = revenue - cost
        return Response({
            "summary": {
