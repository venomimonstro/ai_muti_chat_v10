...
class InfrastructureHealthView(APIView):
    ...
    def get(self, request):
        ...
        try:
            probe = f"infra-probe:{request.user.pk}"
        ...
        disk = _disk_stats()
        memory_warning = memory["used_percent"] is not None and memory["used_percent"] >= 90
        disk_warning = disk["used_percent"] is not None and disk["used_percent"] >= 90

        overall_ok = bool(
            database["ok"]
            and cache_state["ok"]
            and background_ok
            and not memory_warning
            and not disk_warning
        )
        return Response(
            {
                "ok": overall_ok,
