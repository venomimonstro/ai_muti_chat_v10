...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not restore:
            blockers.append("recent isolated restore drill missing")
        if not rollback:
            blockers.append("recent application rollback drill missing")
        for kind in OperationalDrillEvidence.Kind.values:
            exists = OperationalDrillEvidence.objects.filter(
                kind=kind,
                passed_at__gte=cutoff,
            ).exists()
            if not exists:
                blockers.append(f"recent drill missing: {kind}")
        if blockers:
            raise CommandError("Operational drills BLOCKED: " + "; ".join(blockers))
