...
class AutoEconomyPaidChatTests(TestCase):
    def test_funded_user_can_use_economy_preview_stream_and_settlement(self):
        ...
        self.assertEqual(settings_response.data["routing_mode"], Conversation.RoutingMode.ECONOMY)
        ...
        self.assertEqual(preview.data["selected_model"], model.slug)
        self.assertGreater(Decimal(preview.data["estimated_max_rub"]), 0)
        self.assertFalse(preview.data["blocked_by_spend_guard"])
        ...
        self.assertIsNotNone(generation.actual_cost_rub)
        self.assertGreater(generation.actual_cost_rub, 0)
        self.assertLess(user.wallet.available_rub, before)
        self.assertEqual(user.wallet.reserved_rub, Decimal("0.0000"))
