...
export function Sidebar(props:Props){
 ...
 return <aside className={`workspaceSidebar ${collapsed?"collapsed":""}`}>
  ...
  <div className="sideFooter">{!collapsed&&<Link className="walletMini" href="/app/wallet"><span><Icon name="wallet"/>Баланс</span><b>{Number(wallet?.available_rub??0).toFixed(2).replace(".",",")} ₽</b></Link>}<div className="profileRow"><Link href="/app/account" className="profileButton"><span className="avatar"><Icon name="user" size={16}/></span>{!collapsed&&<span>Аккаунт</span>}</Link>{!collapsed&&<Link className="iconButton" href="/app/settings" aria-label="Настройки"><Icon name="settings"/></Link>}</div></div>
 </aside>;
}
