...
export function Sidebar(props:Props){
 ...
 return <aside className={`workspaceSidebar ${collapsed?"collapsed":""}`}>
  ...
  <div className="sideFooter">{!collapsed&&<Link className="walletMini" href="/app/wallet"><span><Icon name="wallet"/>Баланс</span><b>{Number(wallet?.available_rub??0).toFixed(2).replace(".",",")} ₽</b></Link>}<div className="profileRow"><button type="button" className="profileButton" onClick={()=>setProfileOpen(current=>!current)} aria-expanded={profileOpen}><span className="avatar"><Icon name="user" size={16}/></span>{!collapsed&&<span>Профиль</span>}</button>{!collapsed&&<Link className="iconButton" href="/app/settings" aria-label="Настройки"><Icon name="settings"/></Link>}{profileOpen&&<div className="profileMenu"><Link href="/app/compare"><Icon name="scale"/>Сравнение моделей</Link><Link href="/app/images"><Icon name="spark"/>Изображения</Link><div className="contextDivider"/><Link href="/app/account"><Icon name="user"/>Аккаунт</Link><Link href="/app/notifications"><Icon name="bell"/>Уведомления</Link><Link href="/app/projects"><Icon name="folder"/>Проекты</Link><Link href="/app/usage"><Icon name="wallet"/>Использование</Link><Link href="/app/wallet"><Icon name="wallet"/>Баланс и платежи</Link><Link href="/app/settings"><Icon name="settings"/>Настройки</Link><Link href="/app/help"><Icon name="search"/>Помощь</Link><div className="contextDivider"/><button type="button" onClick={()=>void logout()}>Выйти</button></div>}</div></div>
 </aside>;
}
