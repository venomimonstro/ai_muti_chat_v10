...
export function MessageCard({conversationId,message,onConversation}:{conversationId:string;message:ChatMessage;onConversation:(value:Conversation)=>void}){
 ...
 const assistantBody=message.status==="streaming"?<div className="streamingText">{message.content}</div>:<MarkdownMessage content={message.content||"Ответ не получен"}/>;
