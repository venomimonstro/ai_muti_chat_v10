...
export default function LoginPage(){
  const router=useRouter();const [busy,setBusy]=useState(false);const [error,setError]=useState("");const [show,setShow]=useState(false);
  const submit=async(e:FormEvent<HTMLFormElement>)=>{e.preventDefault();setBusy(true);setError("");const f=new FormData(e.currentTarget);try{await ensureCsrf();await api("/auth/login/",{method:"POST",body:JSON.stringify({username:f.get("username"),password:f.get("password")})});router.replace("/app");router.refresh();}catch(r){setError(r instanceof Error?r.message:"Не удалось войти");}finally{setBusy(false);}};
