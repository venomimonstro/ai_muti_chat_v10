...
export default function FAQPage(){return <main className={styles.page}><div className={styles.shell}><header className={styles.top}><div className={styles.brand}>AI Workspace</div><nav className={styles.nav}><Link href="/">Чат</Link><Link href="/pricing">Стоимость</Link><Link href="/getting-started">Начало работы</Link></nav></header><section className={styles.hero}><h1>Частые вопросы</h1><p>Коротко о моделях, оплате, файлах, стабильности и безопасности.</p></section><section className={styles.faq}>{items.map(([q,a])=><details key={q}><summary>{q}</summary><p>{a}</p></details>)}</section></div></main>}