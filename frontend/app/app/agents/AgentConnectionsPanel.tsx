...
export default function AgentConnectionsPanel({agentId}:Props){
 ...
 const bind=async()=>{if(!selectedConnection)return;if(selectedConnection.health_state!=="healthy"){setError("Сначала проверьте подключение. Агенту можно выдать только рабочий внешний сервис.");return}setBusy("bind");setError("");setNotice("");try{await api("/agent-connections/",{method:"POST",body:JSON.stringify({agent:agentId,connection:selectedConnection.id,purpose:"publish",enabled:true})});setSelected("");setNotice("Подключение разрешено этому агенту.");await load()}catch(e){setError(e instanceof Error?e.message:"Не удалось подключить сервис к агенту")}finally{setBusy("")}};
 const remove=async(item:Binding)=>{setBusy(item.id);setError("");setNotice("");try{await api(`/agent-connections/${item.id}/`,{method:"DELETE"});setNotice("Подключение отвязано от агента.");await load()}catch(e){setError(e instanceof Error?e.message:"Не удалось отвязать сервис")}finally{setBusy("")}};
 const check=async(connectionId:string)=>{setBusy(`check:${connectionId}`);setError("");setNotice("");try{const checked=await api<Connection>(`/connections/${connectionId}/check/`,{method:"POST"});setNotice(`${checked.name}: ${checked.health_state==="healthy"?"подключение работает":"проверка завершена"}.`);await load()}catch(e){setError(e instanceof Error?e.message:"Проверка подключения не прошла");await load()}finally{setBusy("")}};
