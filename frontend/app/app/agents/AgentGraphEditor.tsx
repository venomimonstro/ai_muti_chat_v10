...
const types=[
 ["llm","AI-задача"],["research","Исследование"],["web","Интернет"],["files","Файлы"],
 ["image","Изображение"],["review","Проверка"],["approval","Подтверждение"],["publish","Публикация"],
 ["analytics","Аналитика"],["github_read","GitHub: чтение"],["github_write","GitHub: изменение"],
 ["code","Код"],["sandbox","Sandbox / тесты"],["handoff","Передача агенту"],["wait","Ожидание"],["finish","Завершение"],
] as const;
...
export default function AgentGraphEditor({graph,disabled,onSave}:Props){
 ...
 return <div>
  <div style={{display:"grid",gap:10}}>{nodes.map((node,index)=><div key={node.id} style={{display:"grid",gridTemplateColumns:"36px minmax(0,1fr) 180px auto",gap:8,alignItems:"center",border:"1px solid #ddd",borderRadius:13,padding:10}}>
   ...
   <select aria-label={`Тип шага ${index+1}`} value={node.type} onChange={event=>update(index,{type:event.target.value})} disabled={disabled||busy} style={{padding:9,border:"1px solid #ccc",borderRadius:9}}>{types.map(([value,label])=><option key={value} value={value}>{label}</option>)}</select>
