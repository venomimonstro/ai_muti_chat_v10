...
export default function AgentToolPolicy({policy,autonomy,onSave,disabled}:Props){
 ...
 const setApproval=(key:"merge",enabled:boolean)=>setLocal(current=>({...current,[key]:enabled?"approval":"disabled"}));
 const publishMode=String(local.publish||"disabled");
 const save=async()=>{setBusy(true);setError("");try{const next:Policy={...local};if(isDev){next.shell=local.shell==="sandbox"?"sandbox":"disabled";next.merge=local.merge==="approval"?"approval":"disabled";}else{const requested=String(local.publish||"disabled");next.publish=requested==="auto"&&autonomy==="autonomous"?"auto":requested==="approval"?"approval":"disabled";}await onSave(next);}catch(e){setError(e instanceof Error?e.message:"Не удалось сохранить разрешения")}finally{setBusy(false)}};
 return <div>
  ...
  {!isDev&&<div style={{padding:"10px 0",borderBottom:"1px solid #eee"}}><strong>Публикация</strong><small style={{display:"block",opacity:.58,margin:"3px 0 8px"}}>Что делать, когда карта доходит до внешней публикации.</small><select value={publishMode} onChange={e=>setLocal(current=>({...current,publish:e.target.value}))} disabled={disabled||busy} style={{width:"100%",padding:9,border:"1px solid #ccc",borderRadius:9}}><option value="disabled">Запрещено</option><option value="approval">Только после моего подтверждения</option>{autonomy==="autonomous"&&<option value="auto">Полностью автоматически</option>}</select>{publishMode==="auto"&&autonomy==="autonomous"&&<div style={{fontSize:12,marginTop:7,opacity:.68}}>Автопубликация выполнится без дополнительного подтверждения, но только через заранее проверенное подключение и в пределах лимитов агента.</div>}{publishMode==="auto"&&autonomy!=="autonomous"&&<div style={{fontSize:12,marginTop:7,color:"#b33140"}}>Переключите автономность агента на «Автономный» или выберите подтверждение.</div>}</div>}
