...
export default function WalletPage(){
 ...
 const topup=async()=>{setBusy(true);setError("");try{const payment=await api<Payment>("/payments/",{method:"POST",headers:{"Idempotency-Key":`wallet:${crypto.randomUUID()}`},body:JSON.stringify({amount_rub:amount})});void trackProductEvent("payment_started",{amount_rub:amount,payment_id:payment.id});sessionStorage.setItem("pending-payment-id",payment.id);if(payment.confirmation_url)window.location.assign(payment.confirmation_url);else{await load();}}catch(reason){void trackProductEvent("payment_failed",{amount_rub:amount,stage:"create"});setError(reason instanceof Error?reason.message:"Не удалось создать платёж");}finally{setBusy(false);}};

 const requestRefund=async()=>{if(!refundPayment||Number(refundAmount)<1)return;setBusy(true);setError("");try{await api<RefundRequest>("/refund-requests/",{method:"POST",body:JSON.stringify({payment_id:refundPayment,amount_rub:refundAmount,reason:refundReason})});setRefundAmount("");setRefundReason("");await load();}catch(reason){setError(reason instanceof Error?reason.message:"Не удалось отправить заявку на возврат");}finally{setBusy(false);}};
