...
function ImageStudioPanel({models, generations, onChange, onWalletChange, onClose}: {models: ImageModel[]; generations: ImageGeneration[]; onChange: (value: ImageGeneration[]) => void; onWalletChange: () => Promise<void>; onClose: () => void}) {
  ...
  return <Drawer title="Изображения" onClose={onClose}><section className="imageStudio">
    {models.length === 0 ? <div className="emptyPanel"><Icon name="image" size={32}/><h3>Image-модели не настроены</h3><p>Администратор может подключить провайдера и цену в Django admin.</p></div> : <>
      ...
      {generations.length === 0 ? <p className="muted">Здесь появятся созданные изображения.</p> : <div className="imageHistory">{generations.map((generation) => <article key={generation.id}><header><div><b>{generation.model_name}</b><small>{dateLabel(generation.created_at)} · {generation.size} · {generation.quality}</small></div><span className={`statusPill ${generation.state === "failed" ? "danger" : ""}`}>{generation.state === "completed" ? money(generation.actual_cost_rub) : generation.state === "running" ? "В работе" : "Ошибка"}</span></header><p>{generation.prompt}</p>{generation.images.length > 0 && <div className="imageGrid">{generation.images.map((image) => <figure key={image.id}><Image src={image.source_url} alt={image.revised_prompt || generation.prompt} width={1024} height={1024} unoptimized/><figcaption><span>#{image.position + 1} · {(image.size_bytes / 1024).toFixed(0)} КБ</span><a href={image.source_url} target="_blank" rel="noreferrer">Оригинал ↗</a></figcaption></figure>)}</div>}{generation.error_code && <small className="imageError">Код ошибки: {generation.error_code}. Средства не списаны.</small>}</article>)}</div>}
    </>}
  </section></Drawer>;
}
