...
import type {AIModel, ChatMessage, Conversation, FileAsset, Notification, Preference, Project, SearchResult, User, Wallet} from "../lib/types";
import {Icon} from "./components/icons";

type Panel = "search" | "projects" | "files" | "wallet" | "notifications" | "settings" | "support" | "cost" | null;
...
export default function Home() {
  ...
  const loadWorkspace = useCallback(async () => {
    ...
    try {
      ...
      const [chatData, modelData, projectData, fileData, walletData, notificationData, preferenceData] = await Promise.all([
        api<Conversation[]>("/conversations/"), api<AIModel[]>("/models/"), api<Project[]>("/projects/"),
        api<FileAsset[]>("/files/"), api<Wallet>("/wallet/"), api<Notification[]>("/auth/notifications/"), api<Preference>("/auth/preferences/"),
      ]);
      setUser(me); setConversations(chatData); setModels(modelData); setProjects(projectData);
      setFiles(fileData); setWallet(walletData); setNotifications(notificationData); setPreferences(preferenceData);
  ...
  const updateConversation = async (changes: Partial<Pick<Conversation, "selected_model" | "project" | "title">>) => {
    if (!active) return;
  ...
  return <main className="appShell">
    ...
    {panel === "cost" && costMessage?.generation && <Drawer title="Стоимость ответа" onClose={() => setPanel(null)}><div className="costHero"><small>Списано по факту</small><strong>{money(costMessage.generation.cost_rub)}</strong></div><dl className="details"><div><dt>Модель</dt><dd>{costMessage.generation.model}</dd></div><div><dt>Провайдер</dt><dd>{costMessage.generation.provider || "—"}</dd></div><div><dt>Входные токены</dt><dd>{costMessage.generation.input_tokens}</dd></div><div><dt>Выходные токены</dt><dd>{costMessage.generation.output_tokens}</dd></div><div><dt>Correlation ID</dt><dd className="mono">{costMessage.generation.correlation_id}</dd></div></dl></Drawer>}
...
function Empty({icon, title, text}: {icon: "search" | "folder" | "file" | "bell"; title: string; text: string}) {
